...
現実世界では、**ヤマト運輸**や**佐川急便**が**燃料費削減、配達時間短縮**などの課題としてこの問題と毎日向き合っています。
...
また、この問題における「データの混ぜ合わせ」とは、二つのデータ間でルートの区間を交換することです。参考(http://ono-t.d.dooo.jp/GA/GA-order.html)
...
部分写像交叉は、できるだけ多く親の順番を受け継ぐことを目的として、部分区間を交換します。
...
交叉によって一定区間の距離は受け継がれますが、必ずしも交叉の結果がより良い結果を生むわけではありません。

結果的に良くなった子を取得し、再び親として採用します。これを数世代繰り返します。

遺伝的アルゴリズムは、このように確率的な変化に依存するので大抵は"準最適"な結果が得られます。

また、親の集団は多いほど交叉のバリエーションが増えますので、適宜親集団の数も設定します。
